...
plt.title('Models - LSTM')

plt.savefig('{}/Models_RF_Seine.jpg'.format(save_path),dpi=600,bbox_inches='tight')
...
scatter = ax.scatter(lon,lat, c=data.kge_test,cmap='Spectral',s=400,marker='s',vmin=0, vmax=1)

cbar =plt.colorbar(scatter)
cbar.set_label('KGE')
plt.title('KGE - Test Period - LSTM')
...
mask_nc.mask.plot(add_colorbar=False,levels=3,colors=['#737373'],alpha=0.22)
scatter = ax.scatter(lon,lat, c=data.pbias_test,cmap='Spectral',s=400,marker='s',vmin=-30, vmax=30)

cbar =plt.colorbar(scatter)
cbar.set_label('PBIAS %')

plt.title('PBIAS % - Test Period - LSTM')
...
mask_nc.mask.plot(add_colorbar=False,levels=3,colors=['#737373'],alpha=0.22)
scatter = ax.scatter(lon,lat, c=data.r_test,cmap='Spectral',s=400,marker='s',vmin=0, vmax=1)

cbar =plt.colorbar(scatter)
cbar.set_label('Correlation')

plt.title('Correlation - Test Period - LSTM')
plt.savefig('{}/Corr_Seine.jpg'.format(save_path),dpi=600,bbox_inches='tight')
