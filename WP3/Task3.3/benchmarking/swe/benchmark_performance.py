...
for region in regions:
    ...
    for pattern in patterns: 
        ...
        #    print('\t','Perfomance file already exists')
        #    continue 
        ...
        for i in range(len(meta)):
            ...
            sim=pd.read_parquet('../saves/simulations/{}/{}/data/swe_{}.parquet'.format(region,pattern,id))
            sim.index=pd.to_datetime(sim['date'])
            
            obs=pd.read_parquet('../saves/observations/{}/data/swe_{}.parquet'.format(region,id))
            obs.index=pd.to_datetime(obs['date'].dt.date)
