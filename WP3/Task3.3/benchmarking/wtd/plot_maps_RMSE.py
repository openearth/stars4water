...
def plot_performance_map(region,performance: pd.DataFrame,
    ...
    ggplt += pn.scale_fill_gradientn(colors=jet_colors,
                                 name="r",
                                 limits=(0, 2))  # Define limits if necessary
...
models=['tsmp','parflowclm']

regions=['Rhine','Drammen','Danube','Danube','Seine','East_Anglia','Messara','Europe']
...
for region in regions:
    ...
    for model in models:
        ...
        if model=='parflowclm' and region!='Rhine':
            continue
        ...
        ggplt = plot_performance_map(region=region,performance=performance,
                                            title="Water table depth anomalies (m) - {}".format(model),
                                            shapefile=shapefile,
                                            extent=extent,
                                            )
            
        if region=='Europe':
            ggplt += pn.theme(figure_size=(12, 12))
        else:
            ggplt += pn.theme(figure_size=(8, 8))
        
        plot_out = pl.Path("saves/figures/{}/rmse_maps_{}_{}.jpg".format(region,region,model))
