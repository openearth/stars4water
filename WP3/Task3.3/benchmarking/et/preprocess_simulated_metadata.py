...
for region in regions:
    ...
    for pattern in patterns:
        ...
        if pattern=='wflowsbm_era5_europe_30sec' or 'lisflood' in pattern:
            continue

        pattern_directory = pl.Path("{}/{}".format(simulation_directory, pattern))

        moisture_files = np.array([file for file in pattern_directory.iterdir() if file.is_file() and file.stem.split("_")[2] == "et"])
        ...
        dataset=xr.open_dataset(moisture_files[0])['et'].isel(time=0).drop_vars(["time"])
        ...
        for index,row in meta.iterrows():   
            ...
            if 'wflowsbm' in pattern or 'parflowclm' in pattern:
